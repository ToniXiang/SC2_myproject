    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.containsKey('refresh')) {
          await _storage.write(key: _refreshTokenKey, value: responseData['refresh']);
        }
  ...
  static Future<Map<String, dynamic>> authenticatedRequest(
    ...
    try {
      ...
      if (response.statusCode == 401 && retryCount < maxRetries) {
        ...
        final refreshSuccess = await refreshToken();
        if (refreshSuccess) {
          return await authenticatedRequest(method, endpoint, body: body, retryCount: retryCount + 1);
        } else {
