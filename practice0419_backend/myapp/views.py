...
class RegisterView(APIView):
    ...
    def post(self,request):
        ...
        if User.objects.filter(username=username).exists():
            return Response({'message': '使用者名稱重複'}, status=status.HTTP_400_BAD_REQUEST, content_type='application/json; charset=utf-8')
        User.objects.create_user(email=email,username=username, password=password)
        return Response({'message': '註冊成功',},status=status.HTTP_201_CREATED, content_type='application/json; charset=utf-8')
